  ...
  <AnimatedPage>
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-heading font-bold text-white">Hostel Complaints</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[{l:'Open',v:1,c:'#fbbf24'},{l:'In Progress',v:1,c:'#60a5fa'},{l:'Resolved',v:2,c:'#34d399'}].map((s,i)=>(
          <div key={i} className="glass-card p-4 text-center"><p className="text-2xl font-heading font-bold" style={{color:s.c}}>{s.v}</p><p className="text-[11px] font-sans text-white/40">{s.l}</p></div>
        ))}
      </div>
          ...
          <motion.div key={i} className="glass-card p-5" initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.08}}>
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                ...
                <div><h3 className="text-sm font-sans font-semibold text-white/90">{c.t}</h3><p className="text-[11px] font-sans text-white/40 mt-0.5">{c.d}</p><p className="text-[10px] font-sans text-white/30 mt-1">{c.r} · {c.dt}</p></div>
              </div>
