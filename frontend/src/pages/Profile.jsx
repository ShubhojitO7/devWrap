      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="glass-card p-6 flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-sm font-sans text-white/40">{user.email}</p>
        ...
        <div className="glass-card p-5 space-y-4">
          {[{i:<School size={16}/>,l:'College',v:user.college},{i:<BookOpen size={16}/>,l:'Branch',v:user.branch},{i:<User size={16}/>,l:'Semester',v:`Sem ${user.semester}`},{i:<Crown size={16}/>,l:'CGPA',v:user.cgpa}].map((f,i)=>(
            <div key={i} className="flex items-center justify-between py-2 border-b border-white/5">
              <div className="flex items-center gap-3 text-white/40">{f.i}<span className="text-xs font-sans">{f.l}</span></div>
              <span className="text-sm font-sans text-white/80">{f.v}</span>
            </div>
          ))}
        </div>
