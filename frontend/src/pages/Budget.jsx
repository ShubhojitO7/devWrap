        ...
        <div className="space-y-3">
          {[{c:'Food',a:1840,p:43,col:'#fb923c'},{c:'Transport',a:960,p:22,col:'#60a5fa'},{c:'Books',a:620,p:15,col:'#a78bfa'},{c:'Others',a:860,p:20,col:'#fbbf24'}].map((c,i)=>(
            <div key={i} className="flex items-center gap-3">
              <span className="text-xs font-sans text-slate-900/ dark:text-white/ w-20">{c.c}</span>
              ...
              <span className="text-xs font-sans text-slate-900/ dark:text-white/ w-16 text-right">₹{c.a}</span>
=======
  useEffect(() => {
  ...
  return (
    <AnimatedPage>
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="glass-card p-5">
            <p className="text-[11px] font-sans text-slate-900/ dark:text-white/">Total Spent</p>
          ...
          <div className="glass-card p-5">
            <p className="text-[11px] font-sans text-slate-900/ dark:text-white/">Remaining</p>
            <p className="text-2xl font-heading font-bold text-emerald-400 mt-1">₹{data.stats.remaining}</p>
            <p className="text-[10px] font-sans text-slate-900/ dark:text-white/ mt-1">of ₹{data.stats.budget} budget</p>
          </div>
          <div className="glass-card p-5">
            <p className="text-[11px] font-sans text-slate-900/ dark:text-white/">Biggest Category</p>
            <p className="text-2xl font-heading font-bold text-orange-400 mt-1">Food</p>
            <p className="text-[10px] font-sans text-slate-900/ dark:text-white/ mt-1">₹1,840 (43%)</p>
          </div>
        </div>
        ...
        <div className="glass-card p-5">
          ...
          {data.expenses.map((g,gi)=>(
            <div key={gi} className="mb-4">
              <p className="text-[11px] font-sans text-slate-900/ dark:text-white/ mb-2">{g.d}</p>
              {g.items.map((e,i)=>(
                <div key={i} className="flex items-center justify-between py-2.5 border-b border-slate-900/ dark:border-white/">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-sans text-slate-900/ dark:text-white/">{e.t}</p>
                      <p className="text-[10px] font-sans text-slate-900/ dark:text-white/">{e.c}</p>
                    </div>
                  </div>
