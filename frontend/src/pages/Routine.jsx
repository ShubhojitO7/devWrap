...
const Routine = () => (
  <AnimatedPage>
    <div className="space-y-6">
      <h1 className="text-xl font-heading font-bold text-white">Routine & Exams</h1>

      {/* Upcoming Exams */}
      <div>
        <h2 className="text-base font-heading font-semibold text-white mb-3 flex items-center gap-2"><AlertCircle size={16} className="text-crimson-rose"/>Upcoming Exams</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {exams.map((e,i)=>(
            <motion.div key={i} className="glass-card-3d p-5" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.08}}>
              ...
              <h3 className="text-sm font-sans font-semibold text-white/90">{e.s}</h3>
              <div className="mt-2 space-y-1">
                <p className="text-[11px] font-sans text-white/40 flex items-center gap-1.5"><Calendar size={10}/>{e.d}</p>
                <p className="text-[11px] font-sans text-white/40 flex items-center gap-1.5"><Clock size={10}/>{e.t} · {e.r}</p>
              </div>
              ...
              <p className="text-[10px] font-sans text-white/30 mt-1">Preparation: {Math.max(10,100-e.days*4)}%</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      {/* Weekly Timetable */}
      <div>
        <h2 className="text-base font-heading font-semibold text-white mb-3 flex items-center gap-2"><Calendar size={16} className="text-blue-400"/>Weekly Timetable</h2>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {Object.entries(schedule).map(([day, classes], di) => (
            <motion.div key={day} className="glass-card p-4" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:di*0.06}}>
              <h3 className="text-sm font-sans font-bold text-white/80 mb-3 text-center pb-2 border-b border-white/5">{day}</h3>
              <div className="space-y-2">
                {classes.map((cls, ci) => (
                  <div key={ci} className="p-2.5 rounded-xl" style={{background:`${cls.c}08`,borderLeft:`3px solid ${cls.c}`}}>
                    <p className="text-[10px] font-sans text-white/30">{cls.t}</p>
                    <p className="text-xs font-sans font-semibold text-white/80">{cls.s}</p>
                    <p className="text-[10px] font-sans text-white/30">{cls.r}</p>
                  </div>
                ))}
              </div>
            </motion.div>
